...
class InsMem
{
	public:
        ...
        InsMem(string name, string ioDir) {       
			id = name;
			IMem.resize(MemSize);
            ifstream imem; //this is the file object named imem
			string line;
			int i=0;
			imem.open(ioDir + "\\imem.txt"); 
			if (imem.is_open())
			{
                //reading line by line and storing it as string in the variable "line"
				while (getline(imem,line)) 
				{      
					IMem[i] = bitset<8>(line);
					i++;
                    //it would have stored the entire instructions as an array in IMem with each element being 1 byte
				}                    
		...
		bitset<32> readInstr(bitset<32> ReadAddress) {    
            ...
            string byte0 = IMem[addr].to_string();     // First byte
            string byte1 = IMem[addr+1].to_string();   // Second byte
            string byte2 = IMem[addr+2].to_string();   // Third byte
            string byte3 = IMem[addr+3].to_string();   // Fourth byte
            
            string fullInstruction = byte0 + byte1 + byte2 + byte3;
            
			// return bitset<32> val
            return bitset<32>(fullInstruction);
		}     
      
    private:
        vector<bitset<8> > IMem;  //each element is 8-bit bitset
};
...
class DataMem    
{
    public: 
        ...
        bitset<32> readDataMem(bitset<32> Address) {	
            ...
            string byte0 = DMem[addr].to_string();     // First byte
            string byte1 = DMem[addr+1].to_string();   // Second byte
            string byte2 = DMem[addr+2].to_string();   // Third byte
            string byte3 = DMem[addr+3].to_string();   // Fourth byte
            
            string fullData = byte0 + byte1 + byte2 + byte3;
            
			// return bitset<32> val
            return bitset<32>(fullData);
		}
